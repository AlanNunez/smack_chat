...
class ChannelVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        SocketService.instance.getChatMessage { (newMessage) in
            if newMessage.channelId != MessageService.instance.selectedChannel?.id && AuthService.instance.isLoggedIn {
                MessageService.instance.userUnreadChannels.append(newMessage.channelId)
                self.tableView.reloadData()
            }
        }
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if MessageService.instance.userUnreadChannels.count > 0 {
            MessageService.instance.userUnreadChannels = MessageService.instance.userUnreadChannels.filter{ $0 != channel.id }
        }
